...
from module import GenerateOutputVector as gout
...
def GenerateObject(num, threshold, k, vert, d_max, gene_array, attempt):
    tetras = []
    edges = []
    pending_tetras = []  # 注目要素になるのを待機している状態の四面体
    ...
    # 配列に追加
    tetras.append(tetra)
    pending_tetras.append(tetra)

    # 初期化処理終了

    while len(pending_tetras) != 0:

        for tetra in pending_tetras:
            if LP.norm(tetra.centroid - vert) > d_max:
                pending_tetras.pop(0)
                continue
            ...
            for target in range(4):
                ...
                if not tcol.isCollide(processed_tetra, tetras):
                    ...
                    pending_tetras.append(validated_tetra)
